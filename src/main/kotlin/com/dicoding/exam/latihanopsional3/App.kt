...
fun manipulateString(str: String, int: Int): String {
    val digitsString  = str.filter { it.isDigit() }
    val calculate = (digitsString .toIntOrNull() ?: 0) * int
    val prefixString = str.substringBefore(digitsString)
    return when{
        prefixString.isNotEmpty() -> prefixString.plus(calculate)
        else -> str.plus(int)
    }
}
